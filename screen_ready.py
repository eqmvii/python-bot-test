...
while True:
  time.sleep(1.5)
  im = pyautogui.screenshot(region=(566,218, 790, 590))
  im = im.convert('RGB')
  start = time.time()
  ...
  # Started work at confidence 0.96

  for key in all_letters:
    for hit in pyautogui.locateAll("keys/" + key + "_lower_key.png", im, confidence=0.97, grayscale=True):
      results.append({'x': hit.left, 'y': hit.top, 'letter': key, 'width': hit.width, 'color': get_color(im, hit.left, hit.top, hit.width, hit.height)})
    for hit in pyautogui.locateAll("keys/" + key + "_upper_key.png", im, confidence=0.97, grayscale=True):
      results.append({'x': hit.left, 'y': hit.top, 'letter': key, 'width': hit.width, 'color': get_color(im, hit.left, hit.top, hit.width, hit.height)})
  ...
  for letter in y_sorted:
    # 7 turned the As in tals lower lines different
